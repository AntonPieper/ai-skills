...
If you plan to use emulator console simulation later, start the emulator once after creating the AVD. That first boot creates `~/.emulator_console_auth_token`, which the helper uses for `emu-console`.
...
The first Gradle wrapper run on a fresh clone may download the wrapper distribution before the actual build starts.
